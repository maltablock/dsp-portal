...
import { Refund } from "app/shared/typings";
...
export const aggregateStackedPackagesData = ({
  ...
  return accountExtResults.map(stake => {
    ...
    const stakingBalanceFromSelf:number = decomposeAsset(get(groupedStakesAccount, `${identifier}.0.balance`, `0.0000 DAPP`)).amount
    const stakingBalanceFromSelfDappHdl:number = decomposeAsset(get(groupedStakesDappHodl, `${identifier}.0.balance`, `0.0000 DAPP`)).amount

    let refundFromSelf = transformRefundObject(get(groupedRefundsAccount, `${identifier}.0`, undefined))
    let refundFromSelfDappHdl = transformRefundObject(get(groupedRefundsDappHodl, `${identifier}.0`, undefined))
